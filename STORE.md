...
Current release artifact: **`Plasmai-1.2.0.plasmoid`**
...
> Plasmai is a KDE Plasma 6 widget for time tracking. Start and stop timers from the panel, edit the running entry, add manual entries, browse statistics, pin favorites, and see today/week totals against your work contract. A zoomed day sparkline shows work hours, overtime, and sun/moon arcs for your location. Supports Kimai, Clockify, Toggl Track, and SolidTime. API tokens stay in KWallet; settings are shared across widget instances. Translations: EN, DE, FR, ES, IT, NL, PT (BR), PL, UK, RU, JA, ZH (CN).

4. Upload screenshots from `screenshots/` (`01-panel.jpg`, `02-popup.jpg`).
5. Upload **`Plasmai-1.2.0.plasmoid`** as the downloadable content.
...
```bash
git tag -a v1.2.0 -m "Plasmai 1.2.0"

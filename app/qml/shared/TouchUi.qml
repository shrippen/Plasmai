...
/**
 * Central touch sizing for the Plasmai app, ported from the Plasmoid's
 * TouchUi.qml. preference: 0 = auto (follow platform tablet mode), 1 = on, 2 = off.
 * The app has no touchMode setting (always mobile/touch-first), so this
 * resolves via Kirigami.Settings.tabletMode, which is true on Android/Plasma Mobile.
 */
QtObject {
    id: root

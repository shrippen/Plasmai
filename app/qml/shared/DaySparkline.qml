 ...
 *
 * Ported from contents/ui/DaySparkline.qml. The app has no Qt5Compat.GraphicalEffects
 * dependency, so the two GE.OpacityMask uses from the original are replaced:
 *  - the track's rounded-capsule mask is now done per-layer (Rectangle radius +
 *    a Canvas clip() path) instead of masking the whole layered item;
 *  - the sun icon's "soft cutout under header text" mask is dropped (the icon
 *    just renders normally, even where it would have faded under a header label) —
 *    a minor cosmetic simplification, not a feature loss.
 */
Item {
    id: root
    ...
    readonly property int labelHeight: Math.max(12, Math.round(KanteStyle.smallFont.pixelSize + 2))
    ...
    Item {
        ...
        // repaint). No longer punched with header-text cutouts (that relied on
        // Qt5Compat.GraphicalEffects.OpacityMask, unavailable here) — it just
        // renders normally on top of the header when paths cross.
        Item {
            id: sunSlot
    ...
    // No GE.OpacityMask here (Qt5Compat unavailable in the app): the moon-base
    // Rectangle uses its own `radius: height/2`, and the sun-gradient Canvas
    // clips its own drawing to the same rounded-capsule path — both layers end
    // up capsule-shaped without needing to mask the whole stack from outside.
    Item {
        id: track
        anchors.left: parent.left
        anchors.right: parent.right
        ...
        Item {
            ...
            Canvas {
                ...
                onPaint: {
                    ...
                    // Qt Quick's Canvas 2D context doesn't implement clip() on every
                    // Qt version this app targets (throws on Qt 6.7 Android), so this
                    // layer isn't clipped to the capsule shape — only matters for the
                    // full-width polarDay fill below, an edge case above the Arctic
                    // Circle; the moon-base Rectangle underneath still has rounded ends.
                    if (sun.polarDay) {
            ...
            Rectangle {
                ...
                color: Qt.rgba(
                    KanteStyle.highlightColor.r * 0.55 + 0.2,
                    KanteStyle.highlightColor.g * 0.55 + 0.22,
                    KanteStyle.highlightColor.b * 0.55 + 0.35,
            ...
            Repeater {
                ...
                delegate: Rectangle {
                    ...
                    color: segOvertime ? KanteStyle.neutralTextColor : KanteStyle.positiveTextColor
    ...
    Item {
        ...
        Repeater {
            ...
            delegate: Text {
                ...
                color: KanteStyle.textColor
                opacity: 0.55
                font.pixelSize: Math.max(9, KanteStyle.smallFont.pixelSize - 1)
                y: 0
                x: {

...
ColumnLayout {
    ...
    readonly property real filterButtonWidth: Math.max(filterAllMetrics.advanceWidth,
                                                       filterBillableMetrics.advanceWidth,
                                                       filterNonBillableMetrics.advanceWidth)
    ...
    readonly property var weeklyStacks: StatsData.weeklyProjectStacks(
        filteredTimesheets, selectedWeekStart, customersById, Date.now(), 6)
    ...
    readonly property var weekHourData: StatsData.weeklyHourTimeline(
        filteredTimesheets, selectedHourWeekStart, customersById, Date.now(), 8,
        workDayBegin, workDayEnd)
    ...
    readonly property var todayPie: {
        ...
        return StatsData.activityBreakdown(
            filteredTimesheets, day, StatsData.endOfDay(day), Date.now(), 6, customersById)
    }

    readonly property var weekPie: {
        var ws = StatsData.startOfWeek(nowDate)
        return StatsData.activityBreakdown(
            filteredTimesheets, ws, StatsData.endOfWeek(ws), Date.now(), 6, customersById)
    }
    ...
    readonly property int filteredTodaySeconds: StatsData.sumSecondsInRange(
        filteredTimesheets,
        StatsData.startOfDay(nowDate),
        StatsData.endOfDay(nowDate),
        Date.now())

    readonly property int filteredWeekSeconds: StatsData.sumSecondsInRange(
        filteredTimesheets,
        StatsData.startOfWeek(nowDate),
        StatsData.endOfWeek(nowDate),
        Date.now())
    ...
    PlasmaComponents3.Label {
        ...
        opacity: 0.75
        text: i18n("Charts for power users. Tracking stays on the main screen.")
    }
    ...
    ColumnLayout {
        ...
        RowLayout {
            ...
            PlasmaComponents3.ToolButton {
                ...
                Layout.minimumWidth: root.filterButtonWidth
                checkable: true
                autoExclusive: true
    ...
    GridLayout {
        ...
        PlasmaComponents3.Label { text: i18n("Today"); opacity: 0.7 }
        PlasmaComponents3.Label {
            Layout.fillWidth: true
        ...
        PlasmaComponents3.Label { text: i18n("This week"); opacity: 0.7 }
        PlasmaComponents3.Label {
            Layout.fillWidth: true
            horizontalAlignment: Text.AlignRight
            font.bold: true
    ...
    PlasmaComponents3.Label {
        ...
        text: {
            ...
            bits.push(i18n("Axis shows business hours (%1–%2); expands if work falls outside.",
                           root.workDayBegin, root.workDayEnd))
            return bits.join(" ")
        }
    }
